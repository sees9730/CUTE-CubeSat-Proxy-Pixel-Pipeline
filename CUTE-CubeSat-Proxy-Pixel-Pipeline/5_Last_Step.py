...
def infillFrame(frame, frame_fits):
    '''
    Infill a single frame based on the final frame fits
...
# Get the final science-minus-recreated-background and the recreated-background frames from the pickle file
frames_data = readPickleFile('Results/WASP189b_Final_Frames_Pre_Infill_v5.pbz2')
final_frames = frames_data['Final Frames']
background_frames = frames_data['Background Frames']

# Get the final frame fits from the pickle file
fits_data = readPickleFile('Results/WASP189b_Final_Frames_Fits_v5.pbz2')
fits = fits_data['Fits']

# Infill all the images and get the infill error
final_frames_infilled = [infillFrame(frame.copy(), fits[id]) for id, frame in enumerate(final_frames)]

# Remove cosmic rays from frames using lacosmic
...
outputPickleFile(cr_removed_frames, 'WASP189b_Final_Frames_TEST')